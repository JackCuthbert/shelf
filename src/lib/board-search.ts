...
function scoreName(name: string, query: string): number | null {
  ...
  if (normalizedName.length > normalizedQuery.length && normalizedName.startsWith(normalizedQuery.slice(0, 3))) {
...
export function rankApps<T extends SearchableApp>(apps: readonly T[], query: string): T[] {
  if (!query.trim()) return [...apps];
  return apps
    .map((app, index) => ({ app, index, score: scoreName(app.name, query) }))
    .filter((entry): entry is { app: T; index: number; score: number } => entry.score !== null)
