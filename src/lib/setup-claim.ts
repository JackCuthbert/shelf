...
export async function acquireSetupClaim(
  db: Client,
  token: string,
  now = Date.now(),
    ...
    try {
      ...
      try {
        const claim = await tx.execute({
          sql: "SELECT state, token, claimedAt FROM Instance WHERE id = ?",
          args: [claimId],
        })
        ...
        const stale =
          row?.state === "claiming" &&
          now - Date.parse(String(row.claimedAt)) > claimLifetimeMs
        if (
          !canCreateFirstAccount(
            Number(users.rows[0]?.count ?? 0),
            Boolean(row) && !stale,
          )
        ) {
          await tx.rollback()
        ...
        if (row) {
          ...
          await tx.execute({
            sql: "INSERT INTO Instance (id, state, token, claimedAt) VALUES (?, 'claiming', ?, ?)",
            args: [claimId, token, claimedAt],
          })
        }
        await tx.commit()
        return "acquired"
...
export async function finishSetupClaim(
  db: Client,
  token: string,
): Promise<boolean> {
...
export async function releaseSetupClaim(
  db: Client,
  token: string,
): Promise<void> {
  await db.execute({
    sql: "DELETE FROM Instance WHERE id = ? AND state = 'claiming' AND token = ?",
    args: [claimId, token],
  })
}
