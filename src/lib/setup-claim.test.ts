...
import { acquireSetupClaim, claimLifetimeMs, finishSetupClaim, releaseSetupClaim } from "./setup-claim";
...
describe("durable initial setup claim", () => {
  it("allows exactly one concurrent first visitor to claim the instance", async () => {
    ...
    expect(results.every((result) => result !== "acquired" || results.filter((r) => r === "acquired").length === 1)).toBe(true);
  });

  it("reclaims an interrupted claim after its lease expires", async () => {
    const [a] = await freshDatabase();
    await a.execute({ sql: "INSERT INTO Instance(id,state,token,claimedAt) VALUES('singleton','claiming','old-token',?)", args: [new Date(Date.now() - claimLifetimeMs - 1000).toISOString()] });
    ...
    const row = await a.execute("SELECT state FROM Instance WHERE id='singleton'");
  ...
  it("allows a failed auth attempt to release only its own claim", async () => {
    ...
    const stillClaimed = await a.execute("SELECT token FROM Instance WHERE id='singleton'");
    expect(stillClaimed.rows[0]?.token).toBe("owner-token");
    await releaseSetupClaim(a, "owner-token");
    const empty = await a.execute("SELECT id FROM Instance WHERE id='singleton'");
