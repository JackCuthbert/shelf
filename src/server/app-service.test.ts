...
import { createSharedAppService, type AppRepository, type SharedApp } from "./app-service";
...
function setup() {
  ...
  const repository: AppRepository = {
    ...
    create: async (input) => { const app = { ...input, id: String(++nextId), createdAt: new Date(), updatedAt: new Date() }; records.set(app.id, app); return app; },
    update: async (id, input) => { const app = { ...records.get(id)!, ...input, updatedAt: new Date() }; records.set(id, app); return app; },
    delete: async (id) => { const app = records.get(id)!; records.delete(id); return app; },
    countIcon: async (slug) => [...records.values()].filter((app) => app.iconSlug === slug).length,
  };
  const cache = { ensure: vi.fn(async () => true), remove: vi.fn(async () => {}) };
  return { service: createSharedAppService(repository, cache), repository, cache, records };
}
...
describe("shared app service", () => {
  ...
  it("removes the previous icon only after its last app stops referencing it", async () => {
    ...
    const remaining = (await service.list()).find((app) => app.name === "Plex again")!;
  ...
  it("removes a newly downloaded icon when the database create fails", async () => {
    const { service, repository, cache } = setup();
    vi.spyOn(repository, "create").mockRejectedValueOnce(new Error("database unavailable"));
  ...
  it("keeps the existing record unchanged when the replacement icon download fails", async () => {
    ...
    cache.ensure.mockRejectedValueOnce(new Error("offline"));
    await expect(service.update({ id: created.id, ...input, name: "Changed", iconSlug: "jellyfin" })).rejects.toThrow("offline");
