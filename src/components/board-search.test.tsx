...
const apps = [
  ...
  { id: "sonarr", name: "Sonarr", url: "https://sonarr.example", iconSlug: "sonarr" },
];

describe("BoardSearch", () => {
  it("renders a sticky header with the board name, search field, and admin link", () => {
    const html = renderToStaticMarkup(<BoardSearch boardName="Home" apps={apps} />);
  ...
  it("renders each app as a new-tab tile in manual order", () => {
    const html = renderToStaticMarkup(<BoardSearch boardName="Home" apps={apps} />);
    const hrefs = [...html.matchAll(/<a[^>]*href="(https:[^"]+)"[^>]*>/g)].map((match) => match[1]);
  ...
  it("keeps the search field visible on an empty board", () => {
    const html = renderToStaticMarkup(<BoardSearch boardName="Empty" apps={[]} />);
    expect(html).toContain('id="board-search"');
