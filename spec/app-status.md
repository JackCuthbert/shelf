...
- A successful HTTP response of any status code means `up`; connection errors and timeouts mean `down`. A request timeout is three seconds. Status says only that the server received an HTTP response, not that the app is fully functional. A failed check stores a friendly reason such as “Connection refused”, “Host not found”, “Timed out after 3s”, or “TLS certificate error”; a successful check clears it.
...
- The shared app library shows each app's last recorded state and, when the last check failed, its reason. It reads the persisted status only and never triggers a probe.
...
- HTTP response codes count as responding; network errors and timeouts count as not responding with a recorded reason.
- A failed check's reason is visible in the shared app library without triggering a new probe.
