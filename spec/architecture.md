...
Keep board positions in one ordered sequence per board. Moves, assignment removal, and app deletion must leave a deterministic order. There are no coordinates or device-specific layouts.
...
- Only the board owner may create or change that board's assignments, order, name, or default status, or delete the board.
